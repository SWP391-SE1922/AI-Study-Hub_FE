...
import { Shield, Users, FileText, MessageSquare, Activity, LogOut, Menu, X, Sun, Moon, ArrowLeftRight } from 'lucide-react';
...
export function AdminLayout() {
  ...
  const menuItems = [
    { path: '/admin?tab=users', label: 'Người dùng', icon: Users, tab: 'users' },
    { path: '/admin?tab=documents', label: 'Tài liệu', icon: FileText, tab: 'documents' },
    { path: '/admin?tab=chatbot', label: 'Chatbot', icon: MessageSquare, tab: 'chatbot' },
    { path: '/admin?tab=logs', label: 'Logs', icon: Activity, tab: 'logs' },
    { path: '/dashboard', label: 'Xem dạng Sinh viên', icon: ArrowLeftRight },
  ];
      ...
      <header className="fixed top-0 left-0 right-0 h-16 border-b border-border bg-background/95 backdrop-blur z-30 flex items-center justify-between px-4 lg:px-6 lg:pl-64 transition-colors duration-300">
        <div className="flex items-center gap-3">
          ...
          <Link to="/admin" className="flex items-center gap-2.5 lg:hidden">
            <div className="w-9 h-9 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl flex items-center justify-center text-white shadow-md">
              <Shield className="w-4 h-4" />
            </div>
            <span className="font-bold text-lg tracking-tight text-foreground">AI Study Hub Admin</span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 border-l border-border pl-4">
            <Avatar className="w-8 h-8 border border-border">
              <AvatarFallback className="bg-purple-100 dark:bg-purple-950 text-purple-600 dark:text-purple-400 text-xs font-bold">AD</AvatarFallback>
            </Avatar>
        ...
        <div>
          ...
          <div className="h-16 flex items-center px-6 border-b border-border hidden lg:flex">
            <Link to="/admin" className="flex items-center gap-2.5">
              <div className="w-9 h-9 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl flex items-center justify-center text-white shadow-md shadow-purple-500/10">
                <Shield className="w-4 h-4" />
              </div>
              <span className="font-bold text-lg tracking-tight text-foreground">AI Study Hub Admin</span>
            </Link>
